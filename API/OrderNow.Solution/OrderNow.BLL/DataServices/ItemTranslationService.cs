...
namespace OrderNow.BLL.DataServices
{
    public class ItemTranslationService:Service<ItemTranslation>,IitemTranslationService
    {
        ...
        public bool CheckItemNameExistForCategory(string itemName, string language, long itemId, long categoryId, long restaurantId)
        {
            return Queryable()
                .Any(x => x.Language.ToLower() == language.ToLower() &&
                          x.ItemName.ToLower() == itemName.ToLower() &&
                          x.ItemId != itemId && x.Item.CategoryId == categoryId && x.Item.Category.RestaurantId == restaurantId  && !x.Item.IsDeleted);
        }
