...
namespace OrderNow.BLL.DataServices
{
    public class SideItemTranslationService:Service<SideItemTranslation>,ISideItemTranslationService
    {
        ...
        public PagedResultsDto GetAllSideItems(string language, long userId, int page, int pageSize)
        {
            ...
            results.TotalCount = _repository.Query(x => !x.SideItem.IsDeleted && x.Language.ToLower() == language.ToLower() 
            //&& x.SideItem.Restaurant.RestaurantAdminId == userId
            ).Select(x => x.SideItem).Count(x => !x.IsDeleted);
            List<SideItem> sideItems;
            if (pageSize > 0)
                sideItems = _repository.Query(x => !x.SideItem.IsDeleted && x.Language.ToLower() == language.ToLower() 
               // && x.SideItem.Restaurant.RestaurantAdminId == userId
                ).Select(x => x.SideItem)
        ...
        public bool CheckSideItemNameExist(string sideItemName, string language, long sizeId, long restaurantAdminId)
        {
            return Queryable()
                .Any(x => x.Language.ToLower() == language.ToLower() &&
                          ...
                          !x.SideItem.IsDeleted && x.SideItemId != sizeId 
                          //&& x.SideItem.Restaurant.RestaurantAdminId == restaurantAdminId
                          );
        }
